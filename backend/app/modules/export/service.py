...
Was CSVExportService (a class of staticmethods) in app/services/
csv_export_service.py; converted to module-level functions to match the
rest of the refactored modules, with the same exact CSV formatting.
"""
...
from app.modules.export import repository


def export_products(db: Session) -> str:
    products = repository.get_all_products(db)
...
def export_movements(db: Session) -> str:
    ...
    movements = repository.get_all_movements(db)
...
def export_alerts(db: Session) -> str:
    """Exporte les alertes"""
    alerts = repository.get_all_alerts(db)
...
def export_suppliers(db: Session) -> str:
    ...
    suppliers = repository.get_all_suppliers(db)
    ...
    # Headers
    writer.writerow(
        ["ID", "Nom", "Contact", "Email", "Téléphone", "Adresse", "Date Création"]
    )
...
def export_locations(db: Session) -> str:
    ...
    locations = repository.get_all_locations(db)
    ...
    # Headers
    writer.writerow(["ID", "Nom", "Description", "Contrôle Température"])
...
def export_categories(db: Session) -> str:
    ...
    categories = repository.get_all_categories(db)
    ...
    # Headers
    writer.writerow(["ID", "Nom", "Description", "Couleur"])
...
def export_product_lots(db: Session) -> str:
    ...
    lots = repository.get_all_product_lots(db)
    ...
    for lot in lots:
        writer.writerow(
            [
                ...
                lot.received_at.strftime("%Y-%m-%d %H:%M:%S")
                if lot.received_at
                else "",
                lot.notes or "",
            ]
...
def export_all_data(db: Session) -> dict[str, str]:
    ...
    """
    return {
        "products.csv": export_products(db),
        "stock_movements.csv": export_movements(db),
        "alerts.csv": export_alerts(db),
        "users.csv": export_users(db),
        "suppliers.csv": export_suppliers(db),
        "locations.csv": export_locations(db),
        "categories.csv": export_categories(db),
        "product_lots.csv": export_product_lots(db),
    }
