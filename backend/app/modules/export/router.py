...
def export_all_csv(
    ...
    **Accessible UNIQUEMENT par les administrateurs**
    ...
    try:
        zip_content = service.create_zip_export(db)
        ...
        raise HTTPException(status_code=500, detail=f"Erreur lors de l'export: {e!s}")
...
def export_products_csv(
    ...
    try:
        csv_content = service.export_products(db)
...
def export_movements_csv(
    ...
    try:
        csv_content = service.export_movements(db)
...
def export_alerts_csv(
    ...
    try:
        csv_content = service.export_alerts(db)
...
def export_users_csv(
    ...
    try:
        csv_content = service.export_users(db)
...
def export_suppliers_csv(
    ...
    try:
        csv_content = service.export_suppliers(db)
...
def export_locations_csv(
    ...
    try:
        csv_content = service.export_locations(db)

        return StreamingResponse(
            iter([csv_content]),
            media_type="text/csv",
            headers={"Content-Disposition": "attachment; filename=localisations.csv"},
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erreur lors de l'export: {e!s}")
...
def export_categories_csv(
    ...
    try:
        csv_content = service.export_categories(db)
...
def export_lots_csv(
    ...
    try:
        csv_content = service.export_product_lots(db)

        return StreamingResponse(
            iter([csv_content]),
            media_type="text/csv",
            headers={"Content-Disposition": "attachment; filename=lots_produits.csv"},
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erreur lors de l'export: {e!s}")
