...
from app.models.models import MovementType, User
from app.modules.movements import service
from app.modules.movements.schemas import StockMovementCreate, StockMovementResponse
...
def list_movements(
    ...
    skip = (page - 1) * size
    items, total = service.get_movements(
        db,
        skip=skip,
        limit=size,
...
def create_movement(
    ...
    if not PermissionChecker.can_create_movement(current_user):
        raise PermissionDenied(
            "Seuls les admins et techniciens peuvent créer des mouvements"
        )

    movement = service.create_movement(db, data, current_user)
...
@router.get(
    "/{movement_id}", response_model=StockMovementResponse, summary="Détail mouvement"
)
def get_movement(
    movement_id: int,
    db: Session = Depends(get_db),
    ...
    return service.get_movement(db, movement_id)
...
def delete_movement(
    ...
    if not PermissionChecker.can_delete_movement(current_user):
        raise PermissionDenied(
            "Seuls les techniciens et administrateurs peuvent supprimer les mouvements"
        )

    service.delete_movement(db, movement_id)
